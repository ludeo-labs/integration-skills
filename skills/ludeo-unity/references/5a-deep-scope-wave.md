...
> (`5-tracking-restore-orchestrator.md`) **once per wave**, before that wave's capture is implemented.
...
> gate.
>
> **This brief adds no new procedure.** It runs **phase 4 Part B** (`references/4-map-game-objects.md` →
...
- [ ] **Phase 4 Part B procedure** → `references/4-map-game-objects.md` "Part B — Deep-Scope Procedure"
...
2. **Run phase 4 Part B, Steps B1–B7, scoped to those types** (`4-map-game-objects.md`):
...
4. **Append, do not rewrite.** Append one `## Entity: <ObjectType> (wave: N)` block per type to
   `OBJECT_TRACKING.md` (schema in `4-map-game-objects.md` §6); append/extend `## Cross-Entity References`
...
- **The load-bearing guardrail** (`4-map-game-objects.md` §5) — if scoping this wave reveals state an
...
- Procedure: `4-map-game-objects.md` "Part B" (Steps B1–B7) — the steps this brief runs.
- Orchestrator: `5-tracking-restore-orchestrator.md` — dispatches this brief as **task 0 of each wave**,
  then runs the wave's row-review gate before dispatching task 1 (capture).
- **Next in the wave:** task 1 (`5b-implement-object-tracking.md`) captures the rows you just appended.
