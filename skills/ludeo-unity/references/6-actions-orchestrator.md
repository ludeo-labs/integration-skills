...
- **Player-perspective naming + correct attribution.** Name actions from the player's perspective
  ...
  `SetGameplayerId` `[Layer]`, which **must match the id passed to `AddPlayer`** (phase 3) — that is
...
- [ ] **player-id matches the id passed to `AddPlayer`** — via `SetGameplayerId` (phase 3); confirmed
