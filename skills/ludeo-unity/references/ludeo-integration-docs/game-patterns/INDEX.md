...
`SendAction` wiring — lives in phase 6. Action names map to `[SDK]`
`LudeoPlayer.SendAction(string)` (via the `[Layer]` `LudeoController.SendAction`).

---

## Pattern Files Come in Three Shapes
...
- **Gameplay-structure patterns** (bosses) describe a *scripted, stateful encounter* that cuts across
  genre — a fight with phases, cutscenes, a locked arena, summoned adds, and a unique spawn trigger, whose
  behavioral state is normally established by one-shot triggers that restoration must reconstruct without
  re-firing (`bosses.md`).

A streaming-world RPG loads **both** open-world shapes plus the relevant genre file(s). A roguelike
loads `procedural-world.md` plus its combat genre file(s). A game with boss fights loads `bosses.md` on
top of its genre file(s). The axes are **orthogonal** — a game can be procedural *and* level-based, or
level-based *with* bosses; load whatever applies.
...
1. **Check the structural axes first (there are two, and they're orthogonal).**
   - *No per-level scenes?* (open-world RPG, sandbox, MMO, streaming world — `CODE_MAP.session_boundaries`
     ...
     begin and end"), and when you reach object tracking (phases 4–5) also load
...
6. **Use the tracking checklist** to validate completeness after object tracking is implemented (phase 5 · task 1).
...
- What objects and properties typically matter for this genre, with each section tiered by restoration
  ...
  **OPTIONAL** (situational/cosmetic). Types are Unity (`Vector3`, `Quaternion`, …). Used as a
  validation checklist in phase 5 · task 1, not a search guide.
...
- **Most catalog actions are player-scoped** — `Kill` means the *player* killed, `Death` means the
  *player* died; map them only where the player is actor/subject (or, in no-avatar games like RTS, the
  player's owned/issued events) and guard non-player-actor sites in phase 6. **But match/round/race/wave
