...
> **These names are a convention, not SDK constants.** `SendAction` takes an arbitrary string; the strings only acquire meaning once the integrator maps them in **Studio Lab → Global Triggers** (which tracked events start and end each segment). Use these names so the mapping is predictable, and record the mapping as a human action item — until it is done, the actions have no backend effect.
