...
- **Download & include (recommended):** download the self-contained release zip via the authenticated GitHub CLI — `gh release download <tag> -R ludeo-labs/unreal-plugin-releases -p '*.zip'` (the repo is private; the raw `downloadUrl` 404s for anonymous tooling — check `gh auth status` first), extract into `Plugins/LudeoUESDK`, commit directly. The release **bundles the C SDK** at `Source/LudeoSDK/SDK/` — a single download satisfies both components (no separate C SDK step). Prefer this release asset over GitHub's source-archive zip (the latter won't resolve LFS binaries).
