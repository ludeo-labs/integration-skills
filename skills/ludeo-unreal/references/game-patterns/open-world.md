...
(`references/phase-02-mapping.md` → Key Concepts, `references/phase-03-lifecycle.md` →
...
| Ludeo overlay opened mid-play | ❌ for the boundary | Pause via `UGameplayStatics::SetGamePaused` (or the game's own pause primitive), **not** `End` — see `references/phase-03-lifecycle.md` |
...
`references/phase-03-lifecycle.md` describes calling `OpenRoom` "when the match starts loading"
...
- **Sampling is gated by your own in-gameplay flag.** `UpdateWritableObjects` only runs while
  ...
  documented in `references/phase-03-lifecycle.md`). Between `BeginGameplay` and the world
...
- **Use the game's existing pause primitive** if it freezes the sim (e.g. the game's own
  ...
  callback drives it. See `references/phase-03-lifecycle.md` §5.2–§5.4 for full
...
1. Tear down the current Gameplay Session and Room: run the standard teardown chain
   (`EndGameplay` → `RemovePlayer` → `CloseRoom` — see `references/phase-03-lifecycle.md`
   §5.6 Teardown Chain).
...
5. The `RoomReady` handler doubles as the **post-Ludeo-load resume**: unpause → apply restored
   state (two-pass) → `BeginGameplay` (see `references/phase-05-tracking-restore.md` for restoration
   approach; cross-check the Player Flow pause-timing note at the end of
   `references/phase-03-lifecycle.md` §5.7 Player Flow Entry).
...
Class names are representative; map them to the game's actual signals during phase-02.*
...
The `lifecycle_hooks` block of the CODE_MAP produced in `references/phase-02-mapping.md` §3b
...
`references/phase-05-tracking-restore.md`. You also still need the **action catalog** and genre
**tracking checklist** for whichever genre(s) the game blends, produced during phase-06 (actions)
and phase-04 (objects) with the genre files in `references/game-patterns/`.

---

For exact SDK call sequences referenced in this file, see `references/phase-03-lifecycle.md`.
