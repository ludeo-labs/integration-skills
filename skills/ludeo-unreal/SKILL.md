...
**Begin-gate audit (every code-producing phase that touches the lifecycle):** Grep the component for `TryBeginGameplay` and confirm the gate includes a **level-loaded** leg (`bLevelLoaded` or equivalent) in **both** Creator and Player Flow — not just `bRoomReady` + `PlayerHandle.IsSet()` (+ phase). `FLudeoPlayer::BeginGameplay` starts the **video encoder**, and the room opens at level load on purpose, so the SDK legs routinely latch while the world is still streaming and the highlight opens on a loading screen. Also verify the leg is polled **unpaused**, has no timeout/force-begin escape, and that `BeginGameplay` fires immediately on latch rather than behind a ticker. Load-speed-dependent and often visible **only in Player Flow** — check per-leg log timestamps on a cold load. Rules: `references/phase-03-lifecycle.md` §3.2.2; failure mode §8.2c.
