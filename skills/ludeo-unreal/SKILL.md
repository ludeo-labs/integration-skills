...
10. **Verify Steam user is in Ludeo environment.** With the **`ludeo-mcp`** MCP connected, ask the human for the Studio Labs **Game ID** — it is in Studio Labs under **Game Options** (`list_game_environments` requires it), then resolve the environment from the platform rather than asking which one, and record its id in `integration.json` → `sdkSetup.ludeoEnvironmentId` — see `references/ludeo-studio-mcp.md`. Otherwise ask the human: "Has the Steam user been added to the Ludeo Studio Labs environment? Ludeo creation silently fails without this — highlights record but can't convert to Ludeos." Either way, record confirmation in `integration.json` → `sdkSetup.steamUserInEnvironment`. **Then ask, once, whether anyone else needs to make Ludeos** — until the game is live on Ludeo only invited people can capture, and for everyone else it silently doesn't work. Ask it in those terms (not as "project access"): *"anyone else on your team who needs to capture — teammates, QA?"* Invite whoever they name (`invite_user_to_env`, a **write** — name every invitee and wait for a go-ahead; see `references/ludeo-studio-mcp.md`), or ask the human to invite them in Studio Labs. Default: just them.
...
- **`ludeo-mcp`** — Studio Labs platform steps this skill otherwise hands to the human (Step 10's environment check, the environment-binding beta version in Phase 3 §3.16). **Install it from the template if it isn't connected** — it needs a fresh session to come up. Its name carries a deployment suffix, so match the `ludeo-mcp` prefix or look for `list_game_environments` in your tool list. Read `references/ludeo-studio-mcp.md` before the first call: **reads free, writes confirmed**. If it can't be connected, follow that file's manual fallbacks and say so.
...
| `ludeo-mcp` | *URL not final — ask the integrations team* (HTTP, bearer token) | **Studio Labs platform work** the agent otherwise hands to the human — environments + the beta version that binds a build to one | Ask the human to do the step in Studio Labs |
...
- `ludeo-mcp`: Phase 1 (environment lookup) and Phase 3 §3.16 (the beta version that binds the build to an environment). Touchpoints, cadence, and the read/write policy: `references/ludeo-studio-mcp.md` — **reads free, writes confirmed**.
